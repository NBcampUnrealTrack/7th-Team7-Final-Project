# ============================================================
# GY 클라이언트 패키징 (Win64, Development — 패키징 빌드에서 콘솔/SmokeTest 가능)
#   실행: powershell -ExecutionPolicy Bypass -File scripts\package_client.ps1 [옵션]
#   백엔드 값 우선순위: 파라미터 > 환경변수(GY_HOSTED_URL 등, CI가 Secrets에서 주입) > 로컬 ini 값 유지
...
# 값 미지정 시 로컬 ini 의 값을 그대로 사용 (로컬 백엔드 대상 패키징 검증용)
$localIni = Get-Content $IniPath -Raw
if (-not $ServerBaseUrl) { $ServerBaseUrl = ([regex]::Match($localIni, 'ServerBaseUrl="?([^"\r\n]+)"?')).Groups[1].Value }
if (-not $PublishableKey) { $PublishableKey = ([regex]::Match($localIni, 'PublishableKey=([^\r\n]+)')).Groups[1].Value }
if (-not $ServerBaseUrl -or -not $PublishableKey) { throw "ServerBaseUrl/PublishableKey not resolved (param/env/local ini)" }
...
try {
    ...
    # 개발 환경 ini 원복 — 실패해도 로컬 값이 남아있게 보장
