...
FSlateColor SGraphPin_SkillPin::GetPinColor() const
{
	if (IsHovered())
	{
		return FLinearColor(1.f, 1.f, 0.3f);
	}
	if (IsConnected())
	{
...
FVector2D SGraphPin_SkillPin::ComputeDesiredSize(float LayoutScaleMultiplier) const
{
...
{
	DrawPinCircle(OutDrawElements, LayerId, AllottedGeometry, GetPinColor().GetSpecifiedColor());
	return LayerId + 1;
}
