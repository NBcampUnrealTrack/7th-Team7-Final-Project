...
void UWorldSessionComponent::BeginPlay()
{
	...
	if (bStandby)
	{
		Session->RegisterStandby(PublicAddr,
			FGYOnSessionId::CreateUObject(this, &UWorldSessionComponent::OnStandbyRegistered));
		return;
	}
...
void UWorldSessionComponent::OnStandbyRegistered(bool bSuccess, int64 InStandbyId)
{
	UWorld* World = GetWorld();
	if (!IsValid(World)) return;

	if (!bSuccess || InStandbyId <= 0)
	{
	...
	World->GetTimerManager().SetTimer(HeartbeatTimerHandle,
		...
		{
			UGYWorldSessionSubsystem* Session = ResolveSession();
			if (IsValid(Session) && StandbyId > 0)
			{
				Session->StandbyHeartbeat(StandbyId);
			}
...
void UWorldSessionComponent::OnStandbyPollTimer()
{
	...
	Session->PollStandbyAssignment(StandbyId,
		FGYOnSessionId::CreateUObject(this, &UWorldSessionComponent::OnStandbyAssignment));
}

void UWorldSessionComponent::OnStandbyAssignment(bool bSuccess, int64 AssignedWorldId)
{
	bAssignmentPollInFlight = false;

	if (!bSuccess || AssignedWorldId <= 0) return; // 미배정 — 계속 폴링
...
void UWorldSessionComponent::OnAwaitWorldReadyTimer()
{
	...
	if (IsValid(Session) && StandbyId > 0)
	{
		Session->ConsumeStandby(StandbyId);
		StandbyId = 0;
	}
	StartWorldHeartbeat();
}
...
void UWorldSessionComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	if (bEnabled)
	{
		...
		if (IsValid(Session))
		{
			if (StandbyId > 0)
			{
				Session->ConsumeStandby(StandbyId); // 미배정 종료 — 행 정리
			}
			else if (!bStandby || WorldId > 0)
			{
				Session->SetWorldOffline(WorldId);
			}
		}
...
void UWorldSessionComponent::OnHeartbeatTimer()
{
	...
	Session->HeartbeatWorld(WorldId, PublicAddr, PlayerCount);
}

UGYWorldSessionSubsystem* UWorldSessionComponent::ResolveSession() const
{
	UWorld* World = GetWorld();
